                                ...
                                char *p_arg, INT16 int_arg)
{
    ...
    switch (cmd)
    {
        ...
        case BTA_AG_HF_CMD_BRSF:
            ...
            /* Add new 1.7 feature only if remote support it */
            /* In fact we get only the (BTA_AG_PEER_FEAT_HFIND | BTA_AG_PEER_FEAT_S4) bits
               and the result is shift to 2 to set the equivalent
               (BTA_AG_FEAT_HFIND | BTA_AG_FEAT_S4) if needed */
            features |= ((p_scb->peer_features &
                    (BTA_AG_PEER_FEAT_HFIND | BTA_AG_PEER_FEAT_S4)) << 2);
