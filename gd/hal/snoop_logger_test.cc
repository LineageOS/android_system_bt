...
class TestSnoopLoggerModule : public SnoopLogger {
 public:
  TestSnoopLoggerModule(std::string log_path, size_t max_packets_per_file, const std::string& btsnoop_mode)
      : SnoopLogger(std::move(log_path), max_packets_per_file, btsnoop_mode) {}
};
...
TEST_F(SnoopLoggerModuleTest, empty_snoop_log_test) {
  // Actual test
  auto* snoop_looger = new TestSnoopLoggerModule(temp_snoop_log_.string(), 10, SnoopLogger::kBtSnoopLogModeFull);
...
TEST_F(SnoopLoggerModuleTest, disable_snoop_log_test) {
  // Actual test
  auto* snoop_looger = new TestSnoopLoggerModule(temp_snoop_log_.string(), 10, SnoopLogger::kBtSnoopLogModeDisabled);
  TestModuleRegistry test_registry;
  test_registry.InjectTestModule(&SnoopLogger::Factory, snoop_looger);
  test_registry.StopAll();
